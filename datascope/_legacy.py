"""
Legacy v1 scoring engine — all functions from scorer.py.

This module is a temporary bridge: it holds every public function from the
original monolith so that existing tests keep passing while the codebase is
refactored into focused submodules.  It will be deleted when U9 is complete.
"""
...
def normalize_type(v):
    """
    Map a cell's Python type to a canonical bucket for type-consistency scoring.

    int and float are both mapped to ``"numeric"`` so that openpyxl int-vs-float
    noise does not penalise columns that are legitimately all-numeric.
    """
    t = type(v)
    if t in (int, float):
        return "numeric"
...
def write_pdf(rankings_df, profiles_df, chart_recs_df, corr_matrix_df,
    ...
    # Cover-style header
    story.append(Paragraph("Field Story Scorer Report", h1))
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Use this when columns contain mixed types that pandas silently coerces."
        ),
    )
